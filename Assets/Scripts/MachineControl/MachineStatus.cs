using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class MachineStatus
{
    public const float BrakePower = 0.3f;
    public const float Turn = 1f;
    public const float TurnPower = 0.2f;
    public const float TurnSpeed = 150f;
    public const float TurnMovePower = 0.05f;
    public const float FloatTurnSpeed = 0.01f;
    public const float FlyTurnDelay = 0.05f;
    public const float FlyBodyReset = 0.8f;
    public const float FloatBodyReset = 1f;
    public const float LandingBodyReset = 0.1f;
    public const float BodyLock = 0.6f;
    public const float JetLimit = 1f;
    public const float AttackRange = 0.999f;
    public const float StrongTurn = 8f;
    public const float ExplosionWait = 3f;
}
